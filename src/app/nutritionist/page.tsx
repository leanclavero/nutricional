...
export default async function NutritionistPage({
  ...
  // 2. Fetch meals from these patients
  let query = supabase
    .from('meals')
    .select(`
      *,
      patient:profiles!meals_patient_id_fkey(id, email),
      interactions(*)
    `)
      ...
      <header className="sticky top-0 z-40 w-full border-b border-zinc-100 bg-white/80 px-6 py-4 backdrop-blur-md dark:border-white/10 dark:bg-zinc-900/80">
        <div className="mx-auto flex max-w-lg items-center justify-between">
          <h1 className="text-xl font-bold tracking-tight text-zinc-900 dark:text-zinc-50">
            Nutri-Feed <span className="font-light text-zinc-400">Pro</span>
          </h1>
          <form action={logout}>
            <button
              type="submit"
              className="flex items-center gap-2 rounded-lg p-2 text-zinc-500 transition hover:bg-zinc-100 hover:text-red-500 dark:text-zinc-400 dark:hover:bg-zinc-800"
              title="Cerrar Sesión"
            >
              <LogOut size={20} />
            </button>
      ...
      <main className="mx-auto max-w-lg px-6 py-8">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
              Monitorea los registros de tus {patients?.length || 0} pacientes.
            </p>
          </div>
        </div>

        {/* Patient Selection Filter (Simple implementation using current URL) */}
        <div className="mb-8 flex flex-wrap gap-2">
            ...
            {patients?.map((p) => (
                <a 
                    key={p.id}
                    href={`/nutritionist?patientId=${p.id}`}
                    className={`rounded-full px-4 py-2 text-xs font-semibold transition-all ${
          ...
          {!meals || meals.length === 0 ? (
            <div className="mt-10 flex flex-col items-center justify-center text-center">
              ...
              <p className="mt-4 text-zinc-500 dark:text-zinc-400">
                No hay comidas registradas por tus pacientes.
              </p>
            </div>
          ) : (
            meals.map((meal: any) => (
