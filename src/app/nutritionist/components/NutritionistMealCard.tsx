  ...
  return (
    <div className="w-full max-w-lg overflow-hidden rounded-2xl bg-white shadow-lg dark:border dark:border-white/10 dark:bg-zinc-900">
      ...
      <div className="flex items-center gap-3 p-4">
        <div className="h-10 w-10 flex-shrink-0 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold dark:bg-blue-900/30">
          {meal.patient.email[0].toUpperCase()}
        </div>
        <div>
          <p className="text-sm font-bold text-zinc-900 dark:text-zinc-50">{meal.patient.email}</p>
      ...
      {/* Patient's original comment */}
      <div className="p-4">
        {meal.comments && (
          <p className="mb-4 text-sm text-zinc-700 dark:text-zinc-300 italic">
            "{meal.comments}"
          </p>
        )}
        ...
        <div className="space-y-4 border-t border-zinc-100 pt-4 dark:border-white/10">
          <div className="flex items-center gap-4">
            <button
              ...
              className={`flex items-center gap-1 transition ${
                likes.length > 0 ? 'text-red-500' : 'text-zinc-400 hover:text-red-500'
              }`}
            >
              <Heart size={20} fill={likes.length > 0 ? 'currentColor' : 'none'} />
              <span className="text-xs font-medium">{likes.length > 0 ? 'Visto' : 'Dar Visto'}</span>
            </button>
            <div className="flex items-center gap-1 text-zinc-400">
              <MessageCircle size={20} />
              <span className="text-xs font-medium">{nutritionistComments.length} Comentarios</span>
            </div>
          </div>

          {/* List of Nutritionist Comments */}
          {nutritionistComments.length > 0 && (
            <div className="space-y-2">
              {nutritionistComments.map((c) => (
                <div key={c.id} className="rounded-lg bg-zinc-50 p-2 text-sm text-zinc-700 dark:bg-zinc-800/50 dark:text-zinc-300">
                  <span className="font-bold text-blue-600 dark:text-blue-400 mr-2">Feedback:</span>
                  {c.content}
                </div>
              ))}
            </div>
          ...
          <form onSubmit={handleComment} className="mt-4 flex items-center gap-2">
            <input
              ...
              placeholder="Añadir feedback profesional..."
              className="flex-1 rounded-full border border-zinc-200 bg-zinc-50 px-4 py-2 text-sm text-zinc-900 outline-none focus:border-blue-500 dark:border-zinc-800 dark:bg-zinc-950 dark:text-zinc-50"
            />
            <button
              type="submit"
