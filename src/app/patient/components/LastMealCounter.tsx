...
export function LastMealCounter({ lastMealDate }: LastMealCounterProps) {
  ...
  useEffect(() => {
    ...
    const updateCounter = () => {
      const now = new Date()
      // Robust parsing: if it's a string, ensure we handle potential space vs T
      const last = new Date(lastMealDate.replace(' ', 'T'))
      
      // Calculate difference in absolute terms to avoid negative values
      const diffMs = Math.max(0, now.getTime() - last.getTime())
      const hours = Math.floor(diffMs / (1000 * 60 * 60))
      const minutes = Math.floor((diffMs % (1000 * 60 * 60)) / (1000 * 60))

      setIsLongTime(hours >= 4)
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-bold text-zinc-300">
                ({new Date(lastMealDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })})
              </span>
            </div>
